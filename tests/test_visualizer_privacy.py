...
def test_visualizer_privacy_override_can_show_original_region() -> None:
    frame = np.zeros((120, 160, 3), dtype=np.uint8)
    frame[20:100, 40:120] = (40, 180, 220)
    cfg = VisualizationConfig(enabled=False, privacy_mode="person_pixelate", privacy_expand_ratio=0.0)
    vis = Visualizer(stream_id="t0", cfg=cfg)
    det = Detection(bbox=(40.0, 20.0, 120.0, 100.0), confidence=0.9)
    ...
    vis.render(
        ...
        risk_events={},
        fps=15.0,
        privacy_mode_override="none",
    )
    out = vis.get_last_output_frame()

    assert out is not None
    # Overlay labels/bboxes may differ, but the interior stays unredacted.
    assert np.array_equal(out[40:80, 60:100], frame[40:80, 60:100])
